...
export default function RegisterScreen({ navigation }) {
  ...
  if (email && password) {
   ...
   setLoading(true)
      ...
      axios.post('https://demo.screenit.io/api/email/create/user/using/email/mobile', req).then(
      (response) => {
        setLoading(false);
  ...
  return (
    <Background>
      ...
      <Button
        mode="contained"
        onPress={()=>{
          onSignUpPressed(email.value,password.value,name.value,phonenum.value)
        }}
        style={{ marginTop: 24 }}
      >
        Sign Up
